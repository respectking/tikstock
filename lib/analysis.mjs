...
export function prosAndCons(s) {
  ...
  if (num(s.pb) && s.pb > 20)              add(cons, 5, `Priced at ${x(s.pb, 0)} book value. Very little hard asset backing here.`);
  if (num(s.ps) && s.ps > 12)              add(cons, 6, `Priced at ${x(s.ps)} sales, which leaves no room for a stumble.`);

  /* ---- growth ---- */
  if (num(s.rg)) {
    if (s.rg >= 20)      add(pros, 9, `Revenue up ${pctPlain(s.rg)} on the year.`);
    else if (s.rg >= 8)  add(pros, 6, `Revenue growing ${pctPlain(s.rg)} year over year.`);
    else if (s.rg < -5)  add(cons, 8, `Revenue fell ${pctPlain(Math.abs(s.rg))} year over year.`);
    else if (s.rg < 1)   add(cons, 5, `Revenue is flat at ${pct(s.rg)} on the year.`);
  }
  if (num(s.rg5) && s.rg5 >= 10) add(pros, 5, `Has compounded revenue at ${pctPlain(s.rg5)} a year over five years.`);
  if (num(s.eg)) {
    if (s.eg >= 25)       add(pros, 6, `Earnings per share up ${pctPlain(s.eg)}.`);
    else if (s.eg <= -20) add(cons, 7, `Earnings per share down ${pctPlain(Math.abs(s.eg))}.`);
  }

  /* ---- profitability ---- */
  if (num(s.roe)) {
    if (s.roe >= 25)      add(pros, 8, `Earns ${pctPlain(s.roe, 0)} back on shareholder equity.`);
    else if (s.roe >= 15) add(pros, 5, `Return on equity of ${pctPlain(s.roe, 0)}.`);
    else if (s.roe < 0)   add(cons, 9, `Losing money. Return on equity is ${pctPlain(s.roe, 0)}.`);
    else if (s.roe < 8)   add(cons, 5, `Return on equity of only ${pctPlain(s.roe, 0)}.`);
  }
  if (num(s.nm)) {
    if (s.nm >= 20)     add(pros, 7, `${pctPlain(s.nm, 0)} of revenue drops through to net profit.`);
    else if (s.nm < 0)  add(cons, 9, `Unprofitable, with a net margin of ${pctPlain(s.nm)}.`);
    else if (s.nm < 4)  add(cons, 5, `Net margin of ${pctPlain(s.nm)} leaves very little room for error.`);
  }
  ...
  if (num(s.cr)) {
    if (s.cr >= 2)     add(pros, 3, `Current assets cover the near-term bills ${s.cr.toFixed(1)} times over.`);
    else if (s.cr < 1) add(cons, 6, `Current liabilities exceed current assets (ratio ${s.cr.toFixed(2)}).`);
  }

  /* ---- dividend ---- */
  if (num(s.dy) && s.dy > 0) {
    if (s.dy > 8)         add(cons, 6, `A yield of ${pctPlain(s.dy)} is usually the market pricing in a cut.`);
    ...
    if (num(s.payout) && s.payout > 90) add(cons, 5, `Dividend eats ${pctPlain(s.payout, 0)} of earnings, leaving little cushion.`);
  }
  ...
  if (num(s.r52)) {
    if (s.r52 <= -25)     add(cons, 6, `Down ${pctPlain(Math.abs(s.r52))} over the past year.`);
    else if (s.r52 >= 40) add(pros, 3, `Up ${pctPlain(s.r52)} over the past year.`);
  }
...
export function splitAdjustShares(shares) {
  ...
  let factor = 1, adjusted = false;
  for (let i = years.length - 1; i > 0; i--) {
    const hit = splitFactor(shares[years[i]] / shares[years[i - 1]]);
    if (hit) { factor *= hit; adjusted = true; }
    out[years[i - 1]] = shares[years[i - 1]] * factor;
  }
  return { shares: out, adjusted };
}

/* The sentence under the table. Split-adjusted, and it no longer calls a 2%
   drift over five years "dilution". */
export function shareCountNote(shares) {
  const { shares: adj, adjusted } = splitAdjustShares(shares);
  if (!adj) return null;
